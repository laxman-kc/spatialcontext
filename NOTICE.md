...
AirScape's [dataset card](https://huggingface.co/datasets/EmbodiedCity/AirScape-Dataset) reserves original video rights to the source owners. On 6 September 2026, the project owner confirmed permission to publish the AirScape_Train_8918 demonstration and subsequently requested a simpler edit. The [README](README.md#actual-video-test) uses the 16-second edit of the same footage; the original recording was 35 seconds. This permission covers this demonstration, not other AirScape footage, and does not place the underlying video under MIT.

Both recordings remain excluded from Git history and source distributions. Raw footage, sampled frames, the GIF, the poster and earlier private Git history remain in the local maintainer handoff. The [distribution receipt](results/video-demo/distribution.json) identifies the attachments; the historical provenance and numerical results retain their original hashes. Follow the source owners' terms and obtain any needed permission for other uses.
